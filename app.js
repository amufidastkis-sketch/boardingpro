...
const fallbackSeedData = {
  internalAccounts: [{ id: 'ACC-Admin! role: 'Admin! username: 'Admin! password: 'Admin! name: 'Admin! nama: 'Admin! status: 'Aktif', active: true }],
...
function sanitizeTextFields(form) {
  if (!form || form.id === 'account-login-form' || form.id === 'Admin! return;
...
function verifyDocumentAccess(docType, userRole) {
  ...
  const allowedRoles = {
    permit: ['parent', 'student', 'pembina', 'musyrif', 'security', 'ma'had', 'Admin! 'kepsek', 'yayasan'],
    discipline: ['student', 'parent', 'pembina', 'musyrif', 'ma'had', 'Admin! 'kepsek', 'yayasan'],
    invoice: ['parent', 'student', 'ma'had', 'Admin! 'kepsek', 'yayasan'],
    receipt: ['parent', 'student', 'ma'had', 'Admin! 'kepsek', 'yayasan']
  };
  return Boolean(allowedRoles[String(docType || '').toLowerCase()]?.includes(role));
}
const SecurityMasker = {
  canViewFull() { return ['ma'had', 'Admin! 'kepsek', 'yayasan'].includes(String(effectiveRole()).toLowerCase()); },
  phone(value) {
...
const sensitiveDocumentRoles = ['ma'had', 'Admin! 'kepsek', 'yayasan'];
...
function kopSuratHtml() {
  return `<header class="kop-container kop-surat"><div class="logo-wrapper" aria-label="Logo Yayasan"><img src="${KOP_SURAT_LOGO}" alt="Logo"></div><div class="kop-text"><b>SEKOLAH TAHFIDZ KEJURUAN</b><h1>IRMAN SOFRAN</h1><p>Kp. Eurih RT. 004 RW.03 Kel. Tambang Ayam, Kec. Anyar, Kab. Serang, Prov. Banten, 42166, Indonesia | Telp: ${SecurityMasker.phone('0877-7120-0615')}</p><small>Admin! | www.tahfidzkejuruan.org</small></div></header>`;
}
...
state.users.forEach((account) => {
  ...
  if (account.role === 'master_Admin! account.role = 'ma'had';
  delete account.activeRole;
});
const primaryAdmin! = state.users.find((account) => account.username === 'Admin! || state.users.find((account) => account.master);
if (primaryAdmin! {
  primaryAdmin! = "ma'had/Admin!";
  primaryAdmin! = 'ma'had';
  primaryAdmin! = 'Admin!
  primaryAdmin! = 'STKIS —
}
if (!state.users.some((account) => account.role === 'ma'had' || account.role === 'Admin! {
  state.users.push({ id: 'ACC-MASTER-AUTO', name: "ma'had/Admin!", username: 'Admin! password: 'STKIS — role: 'ma'had', status: 'Aktif', active: true, master: true, passwordChangeCount: 0 });
}
...
const documentActionRoles = ['orang_tua', 'parent', 'kepala_sekolah', 'kepsek', 'Admin! 'ma'had', 'ma'had', 'Admin! 'yayasan', 'pengurus_yayasan', 'guru', 'guru_tahfizh'];
...
function exportReportPdf() {
  ...
  const rows = state.students.map((student) => `<tr><td>${escapeHtml(student.name)}</td><td>${escapeHtml(student.className || student.program || '')}</td><td>${escapeHtml(String(student.attendance ?? ''))}%</td><td>${escapeHtml(student.spp || '')}</td></tr>`).join('');
...
const roleDisplayPrefix = (role) => ({ guru: 'Ust.', guru_tahfizh: 'Ust.', pembina: 'Ust.', musyrif: 'Ust.', parent: 'Bpk.', yayasan: 'Dr.', kepsek: 'Drs.', ma'had: 'Admin! ma'had: 'Admin! Admin! 'Admin! }[role] || '');
...
function canViewPkl() {
  return ['ma'had', 'ma'had', 'Admin! 'kepsek', 'yayasan'].includes(effectiveRole()) || (effectiveRole() === 'guru' && pklSpecialistAccount());
}
...
const currentRoleIsAdmin! = () => ['ma'had', 'Admin!
...
const canManageAnnouncements = () => ['ma'had', 'kepsek'].includes(effectiveRole());
const canPromoteStudents = () => ['kepsek', 'ma'had', 'Admin!
const canManageFinance = () => ['ma'had', 'Admin!
const canManageSecurity = () => ['security', 'ma'had', 'Admin!
const canManageDormitory = () => ['ma'had', 'Admin! 'pembina', 'musyrif'].includes(effectiveRole()) || (['guru', 'pembina', 'musyrif'].includes(effectiveRole()) && currentAccount()?.isMusyrif === true);
...
const canManageSchool = () => ['ma'had', 'Admin! 'kepsek'].includes(effectiveRole());
const canViewFinance = () => ['ma'had', 'Admin! 'parent', 'yayasan', 'kepsek', 'pembina', 'musyrif'].includes(effectiveRole());
...
const categoryLabel = (id) => {
  const categories = [financeCategories.spp, financeCategories.foundation, ...financeCategories.ma'hadNonSpp, financeCategories.pocketMoney, financeCategories.custom];
...
function navItems() {
  ...
  const semesterTahfizhItem = ['yayasan', 'kepsek', 'ma'had', 'ma'had', 'Admin! 'pembina', 'musyrif', 'guru_tahfizh'].includes(effectiveRole()) ? { id: 'tahfizh-semester', label: 'Rekap Tahfizh Semester', icon: 'book-open-check' } : null;
  ...
  if (isMasterAdmin! && !state.activeRoleView) {
  ...
  return map[role] || map[role === 'ma'had' ? 'ma'had' : role === 'santri' ? 'student' : role] || common;
}
...
function renderShell() {
  ...
  const emulatorRoles = ['ma'had', 'yayasan', 'kepsek', 'guru', 'pembina', 'security', 'parent', 'student'];
  if (switcher) switcher.innerHTML = isMasterAdmin!
  ...
  const presentationRoles = ['yayasan', 'ma'had', 'Admin! 'kepsek', 'guru', 'guru_tahfizh'];
  ...
  $('#active-role-view')?.addEventListener('change', (event) => {
    state.activeRoleView = event.target.value === 'ma'had' ? null : event.target.value;
...
function commonDashboard(roleTitle, description, extra = '') {
  const role = roles[effectiveRole()] || roles.ma'had || roles.yayasan;
...
function managementView() {
  ...
  return `${welcome('PUSAT KENDALI PENGASUHAN', 'Selamat datang, Admin! ‰¡Æ’Ã¦Ã¯', 'Kelola santri, tagihan, dan laporan ma'had.', currentRoleIsAdmin! ? `<button class="btn btn-primary" data-action="add-student">${icon('plus')} Tambah Santri</button>` : '')}
...
function academicView() {
  const canEdit = ['ma'had', 'Admin! 'kepsek', 'guru'].includes(effectiveRole());
...
function teacherAttendanceAdmin! {
...
function attendanceTable(type = 'kelas') {
  if (type === 'tahfizh' && !hasTahfizhAccess() && !['parent', 'student'].includes(effectiveRole())) return '<div class="notice">Akses presensi jam Tahfizh hanya untuk Guru Tahfizh, Pembina/Musyrif, atau Admin!</div>';
...
function pklTable() {
  return table(['Santri', 'Jam Masuk', 'Jam Pulang', 'Status Absensi', 'Deskripsi Kegiatan', 'Foto'], state.pklReports.map((report) => `<tr><td>${escapeHtml(studentById(report.studentId).name)}</td><td>${escapeHtml(report.checkIn || '')}</td><td>${escapeHtml(report.checkOut || '')}</td><td>${statusBadge(report.status)}</td><td>${escapeHtml(report.activity || '')}</td><td>${report.photo ? 'Tersedia' : ''}</td></tr>`).join(''));
}
...
function selfPasswordForm() {
  const account = currentAccount();
  const locked = !isMasterAdmin! && (account?.passwordChangeCount || 0) >= 2;
...
function disciplineFilters(records) {
  ...
  const rows = visible.map((record) => {
    const student = studentById(record.studentId);
      const canApprove = ['ma'had', 'Admin! 'kepsek'].includes(role) && ['Menunggu Verifikasi', 'Draft'].includes(record.status);
...
function disciplineView() {
  ...
  const canCreate = ['ma'had', 'Admin! 'pembina', 'musyrif'].includes(role);
...
function renderView() {
  ...
  else if (state.view === 'tahfizh-semester') html = ['yayasan', 'kepsek', 'ma'had', 'ma'had', 'Admin! 'pembina', 'musyrif', 'guru_tahfizh'].includes(effectiveRole()) ? tahfizhSemesterView() : dashboard();
  ...
  else if (state.view === 'teacher-attendance') html = (currentRoleIsAdmin! || ['kepsek', 'yayasan'].includes(effectiveRole())) ? teacherAttendanceAdmin! : effectiveRole() === 'guru' ? teacherPersonalAttendanceView() : dashboard();
...
function renderDashboard() {
  try {
    ...
    if (typeof renderTeacherAttendanceModule === 'function') {
      ...
      if (currentRole.includes("ma'had") || currentRole.includes("ma'had") || currentRole.includes("Admin!")) {
...
function accountsTable() {
  return table(['Nama', 'Username', 'Peran', 'Password', 'Aksi'], state.internalAccounts.map((account) => {
    ...
    return `<tr><td><b>${escapeHtml(account.name || account.nama)}</b></td><td>${escapeHtml(account.username)}</td><td>${accountRoles.map((role) => `<span class="badge badge-neutral">${escapeHtml(role)}</span>`).join(' ')}</td><td><code>${escapeHtml(account.password)}</code><small>${account.role === 'ma'had' ? 'Tanpa batas' : `Ganti: ${account.passwordChangeCount || 0}/2`}</small></td><td><button type="button" class="btn btn-small btn-ghost" data-reset-account="${account.id}">Reset Password</button>${account.id !== 'ACC-SUPER' && account.id !== 'ACC-MASTER' ? ` <button type="button" class="btn btn-small btn-ghost" data-delete-account="${account.id}">Hapus Akun</button>` : ''}${isMasterAdmin! && account.role !== 'master_Admin! ? ` <button type="button" class="btn btn-small btn-ghost" data-toggle-account="${account.id}">${account.status === 'Aktif' ? 'Nonaktifkan' : 'Aktifkan'}</button> <button type="button" class="btn btn-small btn-ghost" data-edit-account="${account.id}">Edit Role</button>` : ''}</td></tr>`;
  }).join(''));
}
...
function pklTable() {
  return `<div class="table-responsive table-wrap">${table(['Santri', 'Jam Masuk', 'Jam Pulang', 'Status Absensi', 'Deskripsi Kegiatan', 'Foto'], state.pklReports.map((report) => `<tr><td>${escapeHtml(studentById(report.studentId).name)}</td><td>${escapeHtml(report.checkIn || '')}</td><td>${escapeHtml(report.checkOut || '')}</td><td>${statusBadge(report.status)}</td><td>${escapeHtml(report.activity || '')}</td><td>${report.photo ? `<a href="${escapeHtml(report.photo)}" target="_blank" rel="noopener">Lihat Foto</a>` : ''}</td></tr>`).join(''), 'Belum ada laporan PKL')}</div>`;
}
function pklView() {
...
function tahfizhSemesterRows(semester, academicYear) {
  ...
  return state.students.map((student) => {
    ...
    const percent = total ? ma'hadir / total * 100) : 0;
...
function renderInvoiceModal(invoiceId, print = false, receiptId = null) {
  ...
  const hash = `STKIS — '').slice(0, 16).toUpperCase()}`;
  const institution = state.config?.institution || {};
...
function bindActions() {
  ...
  if (disciplineFormElement) disciplineFormElement.addEventListener('submit', (event) => {
    event.preventDefault();
    if (!['ma'had', 'Admin! 'pembina', 'musyrif'].includes(effectiveRole())) return;
  ...
  document.querySelectorAll('[data-discipline-approve]').forEach((button) => button.addEventListener('click', () => {
    if (!['ma'had', 'Admin! 'kepsek'].includes(effectiveRole())) return;
  ...
  document.querySelectorAll('[data-status]').forEach((button) => button.addEventListener('click', (event) => {
    ...
    const finish = (rejectionReason, approvedDays) => {
      ...
      permit.approvedBy = roles[effectiveRole()]?.label || roles.ma'had.label;
      ...
      if (permit.status === 'Approved') {
        ...
        permit.officialSignature = '[ TERVERIFIKASI DIGITAL ma'had / PEMBINA ]';
        permit.officialStamp = 'STEMPEL DIGITAL ma'had';
      }
      persist();
      render();
    };
  ...
  document.querySelectorAll('[data-delete-account]').forEach((button) => button.addEventListener('click', () => {
    ...
    if (!account || account.role === 'master_Admin! || (account.role === 'ma'had' && !isMasterAdmin! return;
  ...
  document.querySelectorAll('[data-toggle-account]').forEach((button) => button.addEventListener('click', () => { if (!isMasterAdmin! return; const account = state.internalAccounts.find((item) => item.id === button.dataset.toggleAccount); if (!account || account.role === 'master_Admin! return; account.status = account.status === 'Aktif' ? 'Nonaktif' : 'Aktif'; account.active = account.status === 'Aktif'; persist(); render(); }));
  document.querySelectorAll('[data-edit-account]').forEach((button) => button.addEventListener('click', () => {
    ...
    openInputModal('Edit Role Akun', 'Kode role internal', account.role, (role) => {
      ...
      persist();
      render();
    }, { placeholder: 'ma'had, kepsek, guru, musyrif, security' });
  }));
  ...
  const Admin! = $('#Admin!
  if (Admin! Admin! (event) => { event.preventDefault(); const form = new FormData(Admin! const account = currentAccount(); const currentPassword = String(form.get('currentPassword') || ''); const newPassword = String(form.get('newPassword') || ''); if (!account || account.password !== currentPassword) { alert('Password saat ini tidak sesuai.'); return; } if (!isMasterAdmin! && (account.passwordChangeCount || 0) >= 2) { alert('Batas maksimal perubahan kata sandi mandiri telah tercapai (2/2 kali). Untuk melakukan perubahan/reset kata sandi kembali, silakan hubungi Admin! ma'had.'); return; } if (newPassword.length < 10 || newPassword !== String(form.get('confirmPassword') || '')) { alert('Password baru minimal 10 karakter dan harus sama dengan konfirmasi.'); return; } account.password = newPassword; if (!isMasterAdmin! account.passwordChangeCount = (account.passwordChangeCount || 0) + 1; state.currentUser = account; persist(); Admin! alert('Password berhasil diubah.'); });
}
...
function openGradeModal() {
  if (!['ma'had', 'Admin! 'kepsek', 'guru'].includes(effectiveRole())) return;
...
function openTahfizhAttendanceModal() {
  ...
  $('#tahfizh-attendance-form').addEventListener('submit', (event) => {
    ...
    if (!hasTahfizhAccess()) return;
    const form = new FormData(event.target);
    state.attendance.unshift({ id: `ATT-TH-${Date.now()}`, type: 'tahfizh', studentId: form.get('studentId'), date: form.get('date'), status: form.get('status'), by: roles[effectiveRole()]?.label || roles.ma'had.label, note: form.get('note') || '' });
    persist();
    closeModal();
    render();
  });
...
function generatePermitNumber(permitId) {
  ...
  const date = new Date();
  return `${paddedId}/PERMIT-ma'had/${romanMonths[date.getMonth()]}/${date.getFullYear()}`;
}
function renderPermitLetter(permitId, print = false) {
  ...
  const institution = state.config?.institution || {};
  const securityHash = permit.securityHash || `STKIS — '').slice(0, 16).toUpperCase()}`;
...
function showGuruRecapForma'hadAdmin! {
  ...
  if (userRole.includes("ma'had") || userRole.includes("ma'had") || userRole.includes("Admin!") || userRole.includes("pengasuhan")) {
