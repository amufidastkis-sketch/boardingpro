...
function exportReportPdf() {
  ...
  const rows = state.students.map((student) => `<tr><td>${escapeHtml(student.name)}</td><td>${escapeHtml(student.className || student.program || '✓  ?? '✓  || '✓ 
...
function renderDocumentPreview(docType, docData = {}) {
  ...
  const content = docData.html || ({
    discipline: `<h2>${escapeHtml(docData.level || 'SURAT KEDISIPLINAN')}</h2><p>Santri: <b>${escapeHtml(docData.studentName || '✓  class="document-multiline">${escapeHtml(docData.sanction || docData.description || '✓ 
    invoice: `<h2>INVOICE PEMBAYARAN</h2><p>Nomor: <b>${escapeHtml(docData.number || '✓  ${escapeHtml(docData.studentName || '✓  class="document-multiline">${escapeHtml(docData.description || '✓  || '✓ 
    billing: `<h2>SURAT TAGIHAN</h2><p>Nomor: <b>${escapeHtml(docData.number || '✓  ${escapeHtml(docData.studentName || '✓  class="document-multiline">${escapeHtml(docData.description || '✓  || '✓ 
    permit: `<h2>SURAT IZIN SANTRI</h2><p>Santri: <b>${escapeHtml(docData.studentName || '✓  class="document-multiline">${escapeHtml(docData.reason || '✓ 
...
function securityReportsView() {
  const incidentRows = state.incidents.map((item) => `<tr><td><b>${escapeHtml(item.type)}</b><small>${escapeHtml(item.location)}</small></td><td>${statusBadge(item.level)}</td><td>${escapeHtml(item.description)}</td><td>${escapeHtml(item.status || 'Baru')}</td><td>${canManageSecurity() ? `<button class="btn btn-small btn-primary" data-security-status="incident:${item.id}">Tindak Lanjuti</button>` : '✓ 
  const lostRows = state.lostFound.map((item) => `<tr><td><b>${escapeHtml(item.itemName)}</b><small>${escapeHtml(item.photoDescription)}</small></td><td>${escapeHtml(item.location)}</td><td>${formatDate(item.dateTime?.slice(0, 10) || today)}</td><td>${escapeHtml(item.status)}</td><td>${canManageSecurity() ? `<button class="btn btn-small btn-primary" data-security-status="lost:${item.id}">Konfirmasi</button>` : '✓ 
...
function scheduleView() {
  ...
  return `${welcome('JADWAL & AGENDA', 'Jadwal Harian dan Event Mendatang', 'Agenda kegiatan yang diperbarui realtime.', actions)}${section('Jadwal Harian', 'Kegiatan, lokasi, dan penanggung jawab.', activityTimeline())}${section('Event Mendatang', 'Agenda resmi sekolah dan ma✓  eventsTable())}`;
}
...
function studentDashboard() {
  ...
  return commonDashboard('SANTRI ”•– PORTAL PERSONAL', 'Jadwal, nilai, tahfizh, dan pengumuman untuk santri.', `${section('Profil & Progress Saya', `${student.className} ”•– ${student.program}`, `${tahfizhTable(student.id)}${gradeTable(student.id)}`)}${section('Jadwal & Event', 'Agenda harian dan kegiatan ma✓  `${activityTimeline()}${eventsTable()}`)}`);
}
...
function unifiedDashboard() {
  ...
  return `${welcome(`BOARDINGPRO STK IS ”•– ${role.label.toUpperCase()}`, `Assalamu'alaikum, ${role.demoName.split(' ')[0]} ‰’`, `${formatLocalLongDate()} ”•– Satu ruang kendali untuk menjaga amanah.`, canManageAnnouncements() ? `<button class="btn btn-primary" data-action="add-announcement">${icon('plus')} Pengumuman</button>` : '')}
    <div class="stats-grid">${stats}</div>
    <div class="grid-2">${section('Pengumuman & Feed Harian', 'Kabar terbaru ma✓  dan perkembangan santri', `<div class="activity-list">${announcementRows || '<div class="empty">Belum ada pengumuman.</div>'}${feed.map((item) => `<div class="activity"><span class="activity-icon green">${icon(item.type === 'tahfizh' ? 'book-open-check' : 'bell')}</span><div><b>${escapeHtml(item.title)}</b><p>${escapeHtml(item.detail)}</p></div><time>${formatDate(item.date)}</time></div>`).join('')}</div>`, `<button class="btn btn-ghost btn-small" data-view="announcements">Lihat semua</button>`)}
...
function yayasanDashboard() {
  ...
  const pending = state.payments.filter((payment) => payment.status === 'Pending').length;
  return `${welcome(`EXECUTIVE OVERVIEW ”•– ${formatLocalLongDate()}`, "Assalamu'alaikum, Pengelola ‰’", 'Ringkasan kinerja dan kesehatan keuangan BoardingPro STK IS.', `<button class="btn btn-primary" data-action="export">${icon('download')} Export Laporan</button>`)}
  ...
  return `<div class="accordion">${programs.map((program, programIndex) => {
    ...
    return `<details class="accordion-item" ${programIndex === 0 ? 'open' : ''}><summary><span><b>${program.name}</b><small>${program.description}</small></span><span class="badge badge-neutral">${classes.length} kelas</span></summary><div class="accordion-body">${classes.map((klass) => {
      ...
      return `<details class="accordion-class"><summary><span><b>${klass.name}</b><small>Wali: ${klass.wali || '✓  ”•– ${students.length} santri</small></span><span class="progress-label">${pct}%</span></summary><div class="accordion-body"><div class="progress"><span style="width:${pct}%"></span></div>${table(['Santri', 'Target (Juz / Surah / Ayat)', 'Capaian (Juz / Surah / Ayat)', 'Mutqin', 'Progress'], students.map((student) => {
        ...
        return `<tr><td><b>${student.name}</b><small>${student.nis}</small></td><td>Juz ${record.targetJuz || 0} ”•– ${record.targetSurah || '✓  ”•– ${record.targetAyat || '✓  ${record.achievementJuz || student.tahfizh || 0} ”•– ${record.achievementSurah || '✓  ”•– ${record.achievementAyat || '✓  || 'Belum dinilai')}</td><td><div class="progress"><span style="width:${studentPct}%"></span></div><small>${studentPct}%</small></td></tr>`;
...
function managementView() {
  ...
  return `${welcome('PUSAT KENDALI PENGASUHAN', 'Selamat datang, Admin ‰’', 'Kelola santri, tagihan, dan laporan ma✓  currentRoleIsAdmin() ? `<button class="btn btn-primary" data-action="add-student">${icon('plus')} Tambah Santri</button>` : '')}
...
function academicView() {
  ...
  return `${welcome('AKADEMIK & TAHFIZH TERINTEGRASI', 'Rekap pembelajaran & hafalan ‰’œ', 'Nilai akademik dan capaian tahfizh santri dalam satu laporan.', canEdit ? `<button type="button" class="btn btn-primary" data-action="add-grade">${icon('plus')} Input Nilai</button>` : '')}
...
function teacherPersonalAttendanceView() {
  ...
  const report = table(['Tanggal', 'Masuk', 'Pulang', 'Status'], records.map((item) => `<tr><td>${formatDate(item.date)}</td><td>${item.checkIn || '✓  || '✓  'Belum ada riwayat presensi');
...
function openTeacherAttendanceDetail(teacherName) {
  ...
  const rows = records.map((item) => `<tr><td>${formatDate(item.date)}</td><td>${new Date(`${item.date}T00:00:00`).toLocaleDateString('id-ID', { weekday: 'long' })}</td><td>${statusBadge(item.status)}</td><td>${Number(item.hours || 0)} jam</td><td>${escapeHtml(item.note || '✓ 
...
function tahfizhTable(id) {
  ...
  return groupedStudentRecords(data, (item) => `<div class="accordion-record"><b>${escapeHtml(studentById(item.studentId).name)}</b><span>Juz ${escapeHtml(item.juz)} ”•– ${escapeHtml(item.surah)} ”•– ${escapeHtml(item.ayat || '✓ 
}
...
function gradeTable(studentId) {
  ...
  if (studentId || ['parent', 'student'].includes(effectiveRole())) return table(['Santri', 'Mata Pelajaran', 'Nilai', 'Catatan'], records.map((grade) => `<tr><td>${escapeHtml(studentById(grade.studentId).name)}</td><td>${escapeHtml(grade.subject)}</td><td><span class="score">${grade.score}</span></td><td>${escapeHtml(grade.note || '✓ 
  return groupedStudentRecords(records, (grade) => `<div class="accordion-record"><b>${escapeHtml(studentById(grade.studentId).name)}</b><span>${escapeHtml(grade.subject)} ”•– Nilai ${escapeHtml(grade.score)}</span><small>${escapeHtml(grade.note || '✓ 
}
...
function pklTable() {
  return table(['Santri', 'Jam Masuk', 'Jam Pulang', 'Status Absensi', 'Deskripsi Kegiatan', 'Foto'], state.pklReports.map((report) => `<tr><td>${escapeHtml(studentById(report.studentId).name)}</td><td>${escapeHtml(report.checkIn || '✓  || '✓  || '✓  ? 'Tersedia' : '✓ 
}
...
function selfPasswordForm() {
  ...
  const warning = 'Batas maksimal perubahan kata sandi mandiri telah tercapai (2/2 kali). Untuk melakukan perubahan/reset kata sandi kembali, silakan hubungi Admin Ma✓ 
...
function formatSanctionForPrint(value) {
  ...
  const listItems = lines.map((line) => line.replace(/^\s*(?:[-*✓  '')).filter(Boolean);
  const isList = lines.some((line) => /^\s*(?:[-*✓ 
  return isList
...
function disciplineFilters(records) {
  ...
  const rows = visible.map((record) => {
    ...
    return `<tr><td><b>${escapeHtml(student.name)}</b><small>${escapeHtml(student.nis || '')}</small></td><td>${escapeHtml(student.className || '✓ 
  }).join('');
...
function studentsTable() {
  return table(['Santri', 'Program / Kelas', 'Jurusan','Kehadiran', 'SPP', 'Aksi'], state.students.map((student) => { const programLabel = student.program === 'PPTAK' ? 'Program PPTAK (1 Tahun)' : student.program === 'KWNQ' ? 'Program KWNQ (3 Bulan)' : student.program; const classLabel = ['PPTAK', 'KWNQ'].includes(student.program) ? (student.status === 'Alumni Program' ? 'Alumni Program' : 'Program khusus') : student.className; return `<tr><td><div class="person"><span class="avatar">${initials(student.name)}</span><div><b>${escapeHtml(student.name)}</b><small>${escapeHtml(student.parent)} ”•– ${escapeHtml(student.nis)}</small></div></div></td><td>${escapeHtml(programLabel)}<br><small>${escapeHtml(classLabel)}</small></td><td>${escapeHtml((state.majors || majors).find((major) => major.id === student.major)?.name || '✓  ? `<div class="actions-inline"><button type="button" class="icon-btn" data-edit-student="${student.id}">${icon('pencil', 16)}</button><button type="button" class="icon-btn" data-delete-student="${student.id}" title="Hapus">${icon('trash-2', 16)}</button></div>` : '✓  }).join(''));
}
function classesTable() {
  return table(['Program', 'Kelas', 'Jurusan', 'Wali Kelas', 'Santri', 'Aksi'], state.classes.map((klass) => {
    ...
    return `<tr><td>${programLabel}</td><td><b>${klass.programId === 'PPTAK' ? 'Program 1 Tahun' : klass.programId === 'KWNQ' ? 'Program 3 Bulan' : klass.name}</b></td><td>${klass.programId === 'SMK' && major ? major.name : '✓  || '✓  ? `<button class="icon-btn" data-edit-class="${klass.id}">${icon('pencil', 15)}</button>` : '✓ 
  }).join(''));
}
function pocketRoomSummary(students) {
  ...
  const groupsHtml = Object.entries(groups).sort(([a], [b]) => a.localeCompare(b, 'id')).map(([className, classStudents]) => {
    const rows = classStudents.map((student) => {
      ...
      return `<tr><td><b>${escapeHtml(student.name)}</b><small>${escapeHtml(student.nis || '')}</small></td><td>${escapeHtml(student.room || '✓ 
    }).join('');
...
function pocketMoneyView() {
  ...
  const approval = currentRoleIsAdmin() ? table(['Santri', 'Nominal', 'Tanggal', 'Catatan', 'Aksi'], pending.map((item) => `<tr><td>${escapeHtml(studentById(item.studentId).name)}</td><td>${money(item.amount)}</td><td>${formatDate(item.date)}</td><td>${escapeHtml(item.note || '✓  type="button" class="btn btn-small btn-primary" data-pocket-approve="${item.id}">Verifikasi</button></td></tr>`).join(''), 'Tidak ada top up pending') : '';
  const roomSummary = role === 'pembina' || role === 'musyrif' ? pocketRoomSummary(managedStudents) : '';
  return `${welcome('UANG SAKU SANTRI', 'Saldo & transaksi', 'Top up wali diverifikasi admin, pengeluaran langsung mengurangi saldo.', '')}<div class="finance-grid"><div class="finance-tile"><span>Saldo ${escapeHtml(student.name)}</span><b>${money(balance)}</b></div><div class="finance-tile"><span>Total top up</span><b>${money(transactions.filter((item) => item.type === 'Top Up' && item.status !== 'Pending').reduce((sum, item) => sum + Number(item.amount || 0), 0))}</b></div><div class="finance-tile"><span>Total pengeluaran</span><b>${money(transactions.filter((item) => item.type === 'Pengeluaran').reduce((sum, item) => sum + Number(item.amount || 0), 0))}</b></div></div><div class="grid-2">${isParent ? section('Ajukan Top Up', 'Status awal menunggu verifikasi admin', topUpForm) : ''}${expenseForm ? section('Catat Pengeluaran', 'Hanya admin atau pembina', expenseForm) : ''}</div>${approval ? section('Verifikasi Top Up', 'Persetujuan admin', approval) : ''}${roomSummary}${section('Riwayat Uang Saku', 'Mutasi saldo terbaru', table(['Tanggal', 'Jenis', 'Nominal', 'Status', 'Catatan'], transactions.map((item) => `<tr><td>${formatDate(item.date)}</td><td>${escapeHtml(item.type)}</td><td>${money(item.amount)}</td><td>${item.status ? statusBadge(item.status) : statusBadge('Verified')}</td><td>${escapeHtml(item.note || '✓  'Belum ada transaksi'))}`;
}
function teachersTable() {
  return table(['Nama', 'Mapel', 'Peran', 'Telepon', 'Status', 'Aksi'], state.teachers.map((teacher) => `<tr><td><b>${teacher.name}</b></td><td>${teacher.subject}</td><td>${teacher.role}</td><td>${teacher.phone || '✓  ? `<button class="icon-btn" data-edit-teacher="${teacher.id}">${icon('pencil', 15)}</button>` : '✓ 
}
...
function announcementsView() {
  ...
  const rows = state.announcements.map((item) => `<tr><td><b>${item.title}</b><small>${item.detail}</small></td><td>${formatDate(item.date)}</td><td>${item.author}</td><td>${canManageAnnouncements() ? `<div class="actions-inline"><button class="icon-btn" data-edit-announcement="${item.id}" title="Edit">${icon('pencil', 15)}</button><button class="icon-btn" data-delete-announcement="${item.id}" title="Hapus">${icon('trash-2', 15)}</button></div>` : '✓ 
...
function promoteStudents() {
  if (!canPromoteStudents()) {
    alert('Kenaikan kelas hanya dapat diproses oleh Ma✓  / Admin atau Kepala Sekolah.');
    return;
  }
...
function pklViewWithEligibility() {
  ...
  if (effectiveRole() === 'student' && !isPklEligible(student)) {
    return section('Praktik Kerja Lapangan', 'Modul belum dibuka', `<div class="notice">${icon('lock', 25)}<b>Akses PKL belum tersedia</b><p class="metric-note">PKL hanya dibuka untuk siswa SMK kelas 11 semester 2 atau kelas 12 semester 1. Status Anda saat ini: kelas ${escapeHtml(student.grade || '✓  semester ${escapeHtml(student.semester || '✓ 
  }
  return pklView();
}

function pklTable() {
  return `<div class="table-responsive table-wrap">${table(['Santri', 'Jam Masuk', 'Jam Pulang', 'Status Absensi', 'Deskripsi Kegiatan', 'Foto'], state.pklReports.map((report) => `<tr><td>${escapeHtml(studentById(report.studentId).name)}</td><td>${escapeHtml(report.checkIn || '✓  || '✓  || '✓  ? `<a href="${escapeHtml(report.photo)}" target="_blank" rel="noopener">Lihat Foto</a>` : '✓  'Belum ada laporan PKL')}</div>`;
}
function pklView() {
...
function attendanceSemesterView() {
  ...
  const rows = state.students.map((student) => {
    ...
    return `<tr><td>${escapeHtml(student.name)}</td><td>${escapeHtml(student.className || '✓ 
  }).join('');
...
function renderInvoiceModal(invoiceId, print = false, receiptId = null) {
  ...
  const cleanRows = `<tr><td>Nomor</td><td>${escapeHtml(source.number || source.id)}</td></tr><tr><td>Santri</td><td>${escapeHtml(student.name)} (${escapeHtml(SecurityMasker.identity(student.nis || '✓  || categoryLabel(source.category))}</td></tr>${detailRows}<tr><td>Status</td><td>${paid ? 'LUNAS / PAID' : 'BELUM LUNAS / UNPAID'}</td></tr>`;
  ...
  document.querySelectorAll('[data-status]').forEach((button) => button.addEventListener('click', (event) => {
    ...
    const finish = (rejectionReason, approvedDays) => {
      ...
      if (permit.status === 'Approved') {
        ...
        permit.officialSignature = '[ TERVERIFIKASI DIGITAL MA✓  / PEMBINA ]';
        permit.officialStamp = 'STEMPEL DIGITAL MA✓ 
      }
      persist();
      render();
    };
