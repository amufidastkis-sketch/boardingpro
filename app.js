...
function securityAlertBanner() {
  ...
  return active.length ? `<div class="notice" style="margin-bottom:18px"><b>âš  ${active.length} laporan darurat keamanan perlu ditindaklanjuti.</b> <button class="btn btn-small btn-ghost" data-view="security-reports">Buka laporan</button></div>` : '';
}
Object.keys(seedData).forEach((key) => { if (!state[key]) state[key] = clone(seedData[key]); });
['classes', 'teachers', 'financeBills', 'scholarships', 'discounts', 'invoices', 'receipts', 'dailyFeed', 'monthlySummaries', 'yayasanProgress', 'internalAccounts', 'announcements', 'pocketTransactions', 'pocketBalances', 'majors', 'teacherTeachingRecords', 'incidents', 'lostFound', 'disciplineRecords'].forEach((key) => {
...
function renderDocumentPreview(docType, docData = {}) {
  ...
  const content = docData.html || ({
    discipline: `<h2>${escapeHtml(docData.level || 'SURAT KEDISIPLINAN')}</h2><p>Santri: <b>${escapeHtml(docData.studentName || 'â€”')}</b></p><p class="document-multiline">${escapeHtml(docData.sanction || docData.description || 'â€”')}</p>`,
    invoice: `<h2>INVOICE PEMBAYARAN</h2><p>Nomor: <b>${escapeHtml(docData.number || 'â€”')}</b></p><p>Santri: ${escapeHtml(docData.studentName || 'â€”')}</p><p class="document-multiline">${escapeHtml(docData.description || 'â€”')}</p><h3>${escapeHtml(docData.total || 'â€”')}</h3>`,
    billing: `<h2>SURAT TAGIHAN</h2><p>Nomor: <b>${escapeHtml(docData.number || 'â€”')}</b></p><p>Santri: ${escapeHtml(docData.studentName || 'â€”')}</p><p class="document-multiline">${escapeHtml(docData.description || 'â€”')}</p><h3>${escapeHtml(docData.total || 'â€”')}</h3>`,
    permit: `<h2>SURAT IZIN SANTRI</h2><p>Santri: <b>${escapeHtml(docData.studentName || 'â€”')}</b></p><p class="document-multiline">${escapeHtml(docData.reason || 'â€”')}</p>`
...
const financeGroupKeys = ['Reguler SMK Â· Kelas 10', 'Reguler SMK Â· Kelas 11', 'Reguler SMK Â· Kelas 12', 'Program PPTAK (1 Tahun) Â· Non-Jenjang', 'Program KWNQ (3 Bulan) Â· Non-Jenjang'];
function groupedActiveStudentRecords(records, renderItem, emptyMessage = 'Belum ada data tagihan') {
  ...
  records.forEach((record) => {
    ...
    const key = `${program} Â· ${['PPTAK', 'KWNQ'].includes(student.program) ? 'Non-Jenjang' : `Kelas ${Number(student.grade)}`}`;
...
function navItems() {
  ...
  const pklItem = isPklEligible(currentStudent()) ? { id: 'pkl', label: 'Praktik Kerja Lapangan', icon: 'briefcase-business' } : { id: 'pkl', label: 'PKL Â· Terkunci', icon: 'lock-keyhole' };
  const map = {
...
function teacherDashboard() {
  ...
  return commonDashboard('GURU / USTADZ Â· AKADEMIK', 'Kelola presensi mandiri dan perkembangan santri di kelas yang diampu.', `${teacherPersonalAttendanceView()}${section('Laporan Santri', 'Pilih modul laporan yang ingin ditinjau.', `<div class="actions-inline" style="margin-bottom:16px">${tabs}</div>${panels}`)}`);
}

function guardianDashboard() {
  return commonDashboard('PEMBINA Â· PENGASUHAN & TAHFIZH', 'Pantau aktivitas asrama, kedisiplinan, dan setoran hafalan santri.', `${pembinaView()}${section('Jadwal & Event', 'Agenda boarding school untuk seluruh role.', `${activityTimeline()}${eventsTable()}`)}`);
}

function securityDashboard() {
  return commonDashboard('SECURITY Â· POS JAGA', 'Pantau izin yang telah disetujui dan aktivitas keluar-masuk gerbang.', `${securityQuickActions()}${gateView()}${section('Feed Aktivitas Santri', 'Pembaruan kegiatan terbaru.', activityList())}`);
}
...
function securityReportsView() {
  const incidentRows = state.incidents.map((item) => `<tr><td><b>${escapeHtml(item.type)}</b><small>${escapeHtml(item.location)}</small></td><td>${statusBadge(item.level)}</td><td>${escapeHtml(item.description)}</td><td>${escapeHtml(item.status || 'Baru')}</td><td>${canManageSecurity() ? `<button class="btn btn-small btn-primary" data-security-status="incident:${item.id}">Tindak Lanjuti</button>` : 'â€”'}</td></tr>`).join('');
  const lostRows = state.lostFound.map((item) => `<tr><td><b>${escapeHtml(item.itemName)}</b><small>${escapeHtml(item.photoDescription)}</small></td><td>${escapeHtml(item.location)}</td><td>${formatDate(item.dateTime?.slice(0, 10) || today)}</td><td>${escapeHtml(item.status)}</td><td>${canManageSecurity() ? `<button class="btn btn-small btn-primary" data-security-status="lost:${item.id}">Konfirmasi</button>` : 'â€”'}</td></tr>`).join('');
...
function scheduleView() {
  ...
  return `${welcome('JADWAL & AGENDA', 'Jadwal Harian dan Event Mendatang', 'Agenda kegiatan yang diperbarui realtime.', actions)}${section('Jadwal Harian', 'Kegiatan, lokasi, dan penanggung jawab.', activityTimeline())}${section('Event Mendatang', 'Agenda resmi sekolah dan maâ€™had.', eventsTable())}`;
}

function parentDashboard() {
  return commonDashboard('ORANG TUA / WALI Â· PORTAL PERSONAL', 'Ringkasan personal anak, akademik, tahfizh, dan keuangan.', `${parentView()}${section('Keuangan Anak', 'Tagihan, pembayaran, invoice, dan uang saku milik anak.', financeView())}`);
}

function studentDashboard() {
  const student = currentStudent();
  return commonDashboard('SANTRI Â· PORTAL PERSONAL', 'Jadwal, nilai, tahfizh, dan pengumuman untuk santri.', `${section('Profil & Progress Saya', `${student.className} Â· ${student.program}`, `${tahfizhTable(student.id)}${gradeTable(student.id)}`)}${section('Jadwal & Event', 'Agenda harian dan kegiatan maâ€™had.', `${activityTimeline()}${eventsTable()}`)}`);
}

function executiveDashboard() {
  const title = state.role === 'kepsek' ? 'KEPALA SEKOLAH Â· EXECUTIVE READ-ONLY' : 'YAYASAN Â· EXECUTIVE READ-ONLY';
...
function unifiedDashboard() {
  ...
  return `${welcome(`BOARDINGPRO STK IS Â· ${role.label.toUpperCase()}`, `Assalamu'alaikum, ${role.demoName.split(' ')[0]} ðŸ‘‹`, 'Satu ruang kendali untuk menjaga amanah.', canManageAnnouncements() ? `<button class="btn btn-primary" data-action="add-announcement">${icon('plus')} Pengumuman</button>` : '')}
    <div class="stats-grid">${stats}</div>
    <div class="grid-2">${section('Pengumuman & Feed Harian', 'Kabar terbaru maâ€™had dan perkembangan santri', `<div class="activity-list">${announcementRows || '<div class="empty">Belum ada pengumuman.</div>'}${feed.map((item) => `<div class="activity"><span class="activity-icon green">${icon(item.type === 'tahfizh' ? 'book-open-check' : 'bell')}</span><div><b>${escapeHtml(item.title)}</b><p>${escapeHtml(item.detail)}</p></div><time>${formatDate(item.date)}</time></div>`).join('')}</div>`, `<button class="btn btn-ghost btn-small" data-view="announcements">Lihat semua</button>`)}
    ...
    ${section('PKL Â· Praktik Kerja Lapangan', 'Monitoring eligibility, penempatan, progress, dan pembimbing', `${pklTable()}<div class="actions-inline"><button class="btn btn-ghost btn-small" data-view="pkl">Buka modul PKL</button></div>`)}`;
}
...
function yayasanDashboard() {
  ...
  const pending = state.payments.filter((payment) => payment.status === 'Pending').length;
  return `${welcome('EXECUTIVE OVERVIEW Â· SABTU, 05 SEPTEMBER 2026', "Assalamu'alaikum, Pengelola ðŸ‘‹", 'Ringkasan kinerja dan kesehatan keuangan BoardingPro STK IS.', `<button class="btn btn-primary" data-action="export">${icon('download')} Export Laporan</button>`)}
    ...
    ${section('Monitoring Yayasan Â· Tahfizh', 'Program â†’ kelas â†’ santri dengan target dan capaian', yayasanAccordion())}`;
}
  ...
  return `<div class="accordion">${programs.map((program, programIndex) => {
    ...
    return `<details class="accordion-item" ${programIndex === 0 ? 'open' : ''}><summary><span><b>${program.name}</b><small>${program.description}</small></span><span class="badge badge-neutral">${classes.length} kelas</span></summary><div class="accordion-body">${classes.map((klass) => {
      ...
      return `<details class="accordion-class"><summary><span><b>${klass.name}</b><small>Wali: ${klass.wali || 'â€”'} Â· ${students.length} santri</small></span><span class="progress-label">${pct}%</span></summary><div class="accordion-body"><div class="progress"><span style="width:${pct}%"></span></div>${table(['Santri', 'Target (Juz / Surah / Ayat)', 'Capaian (Juz / Surah / Ayat)', 'Mutqin', 'Progress'], students.map((student) => {
        ...
        return `<tr><td><b>${student.name}</b><small>${student.nis}</small></td><td>Juz ${record.targetJuz || 0} Â· ${record.targetSurah || 'â€”'} Â· ${record.targetAyat || 'â€”'}</td><td>Juz ${record.achievementJuz || student.tahfizh || 0} Â· ${record.achievementSurah || 'â€”'} Â· ${record.achievementAyat || 'â€”'}</td><td>${statusBadge(record.mutqin || 'Belum dinilai')}</td><td><div class="progress"><span style="width:${studentPct}%"></span></div><small>${studentPct}%</small></td></tr>`;
...
function managementView() {
  ...
  return `${welcome('PUSAT KENDALI PENGASUHAN', 'Selamat datang, Admin ðŸ‘‹', 'Kelola santri, tagihan, dan laporan maâ€™had.', currentRoleIsAdmin() ? `<button class="btn btn-primary" data-action="add-student">${icon('plus')} Tambah Santri</button>` : '')}
...
function academicView() {
  ...
  return `${welcome('AKADEMIK & TAHFIZH TERINTEGRASI', 'Rekap pembelajaran & hafalan ðŸ“š', 'Nilai akademik dan capaian tahfizh santri dalam satu laporan.', canEdit ? `<button type="button" class="btn btn-primary" data-action="add-grade">${icon('plus')} Input Nilai</button>` : '')}
...
function teacherPersonalAttendanceView() {
  ...
  const report = table(['Tanggal', 'Masuk', 'Pulang', 'Status'], records.map((item) => `<tr><td>${formatDate(item.date)}</td><td>${item.checkIn || 'â€”'}</td><td>${item.checkOut || 'â€”'}</td><td>${statusBadge(item.status)}</td></tr>`).join(''), 'Belum ada riwayat presensi');
...
function teacherAttendanceTable() {
  ...
  const rows = teachers.map((teacher) => {
    ...
    const subject = items[0]?.subject || 'â€”';
...
function openTeacherAttendanceDetail(teacherName) {
  ...
  const subject = records[0]?.subject || 'â€”';
  const rows = records.map((item) => `<tr><td>${formatDate(item.date)}</td><td>${new Date(`${item.date}T00:00:00`).toLocaleDateString('id-ID', { weekday: 'long' })}</td><td>${statusBadge(item.status)}</td><td>${Number(item.hours || 0)} jam</td><td>${escapeHtml(item.note || 'â€”')}</td></tr>`).join('');
  openModal(`Detail Presensi Â· ${teacherName}`, `<p class="metric-note">Mata Pelajaran: ${escapeHtml(subject)} Â· September 2026</p>${table(['Tanggal', 'Hari', 'Status', 'Jam Mengajar', 'Catatan Materi KBM'], rows, 'Belum ada histori harian')}`);
}
function pembinaView() {
  return `${welcome('PENGAWASAN ASRAMA & TAHFIZH', 'Dashboard Pembina ðŸ›¡ï¸', 'Pantau kedisiplinan, presensi jamaah, dan hafalan santri binaan.', canManageDormitory() ? `<button class="btn btn-primary" data-action="add-point">${icon('plus')} Input Poin</button><button class="btn btn-primary" data-action="add-tahfizh-attendance">${icon('plus')} Absensi Tahfizh</button>` : '')}
...
function parentView() {
  ...
  return `<div class="child-hero"><div class="avatar xl">${initials(child.name)}</div><div><span class="eyebrow">PORTAL ORANG TUA / WALI</span><h1>${child.name}</h1><p>${child.className} Â· ${child.room} Â· ${child.program} Â· NIS ${child.nis}</p></div><span class="badge badge-success">Santri Aktif</span></div>
...
function studentView() {
  const student = currentStudent();
  return `${welcome('PORTAL SANTRI Â· PERSONAL DASHBOARD', `Assalamu'alaikum, ${student.name.split(' ')[0]}! ðŸ‘‹`, 'Semangat menjalani aktivitas hari ini.', '')}
...
function pklSummary(student) {
  ...
  return report ? `<div><div class="notice"><span class="status-dot"></span>${report.status} Â· ${report.company}</div><p class="metric-note">Progress laporan ${report.progress}% Â· Kehadiran ${report.attendance}%</p><button class="btn btn-ghost" data-view="pkl">Buka modul PKL ${icon('arrow-right')}</button></div>` : '<div class="empty">Belum ada penempatan PKL.</div>';
}
function gateView() {
  const active = state.permits.filter((permit) => permit.status === 'Approved' && permit.date === today && permit.checkout && !permit.checkin).length;
  const requestForm = effectiveRole() === 'security' && canManageSecurity() ? `<form id="security-permit-form" class="form-grid"><label>Santri<select name="studentId">${state.students.map((student) => `<option value="${student.id}">${escapeHtml(student.name)} Â· ${escapeHtml(student.nis)}</option>`).join('')}</select></label><label>Jenis Izin<select name="type"><option value="Keluar Kompleks">Keluar Kompleks</option><option value="Pulang / Mudik">Pulang / Mudik</option></select></label><label>Tanggal<input name="date" type="date" value="${today}" required></label><label class="full">Alasan<textarea name="reason" required></textarea></label><button type="submit" class="btn btn-primary full">Catat Pengajuan di Pos</button></form>` : '';
  return `${welcome(`POS JAGA UTAMA Â· ${today}`, 'Kontrol Gerbang ðŸšª', 'Catat pergerakan santri secara realtime dan aman.')}
...
function permitGroupClass(student) {
  return ['PPTAK', 'KWNQ'].includes(student.program) ? 'Non-Jenjang' : [10, 11, 12].includes(Number(student.grade)) ? `Kelas ${Number(student.grade)}` : 'Kelas â€”';
}
...
function groupedStudentRecords(records, renderItem) {
  ...
  visible.filter((record) => isActiveStudent(studentById(record.studentId))).forEach((record) => {
    ...
    const key = `${program} Â· ${permitGroupClass(student)}`;
...
function tahfizhTable(id) {
  ...
  if (id || isPersonalRole) return table(['Santri', 'Juz / Surah', 'Ayat', 'Jenis', 'Status', ...(actionHeader ? [actionHeader] : [])], data.map((item) => `<tr><td>${escapeHtml(studentById(item.studentId).name)}</td><td><b>Juz ${item.juz}</b> Â· ${escapeHtml(item.surah)}</td><td>${escapeHtml(item.ayat)}</td><td>${escapeHtml(item.type)}</td><td>${statusBadge(item.status)}</td>${actionHeader ? `<td><button type="button" class="icon-btn" data-edit-tahfizh="${item.id}" title="Edit setoran">${icon('pencil', 15)}</button></td>` : ''}</tr>`).join(''));
  return groupedStudentRecords(data, (item) => `<div class="accordion-record"><b>${escapeHtml(studentById(item.studentId).name)}</b><span>Juz ${escapeHtml(item.juz)} Â· ${escapeHtml(item.surah)} Â· ${escapeHtml(item.ayat || 'â€”')}</span>${statusBadge(item.status)}</div>`);
}
...
function gradeTable(studentId) {
  ...
  if (studentId || ['parent', 'student'].includes(effectiveRole())) return table(['Santri', 'Mata Pelajaran', 'Nilai', 'Catatan'], records.map((grade) => `<tr><td>${escapeHtml(studentById(grade.studentId).name)}</td><td>${escapeHtml(grade.subject)}</td><td><span class="score">${grade.score}</span></td><td>${escapeHtml(grade.note || 'â€”')}</td></tr>`).join(''));
  return groupedStudentRecords(records, (grade) => `<div class="accordion-record"><b>${escapeHtml(studentById(grade.studentId).name)}</b><span>${escapeHtml(grade.subject)} Â· Nilai ${escapeHtml(grade.score)}</span><small>${escapeHtml(grade.note || 'â€”')}</small></div>`);
}
function attendanceTable(type = 'kelas') {
  ...
  return groupedStudentRecords(data, (item) => `<div class="accordion-record"><b>${escapeHtml(studentById(item.studentId).name)}</b><span>${formatDate(item.date)} Â· ${item.status}</span><small>${escapeHtml(item.by || 'Pembina')}</small></div>`);
}
function pklTable() {
...
function billingTable(limit, studentId) {
  ...
  const renderBill = (bill) => `<div class="accordion-record"><b>${escapeHtml(studentById(bill.studentId).name)}</b><span>${escapeHtml(bill.label)} Â· ${money(billNet(bill))}</span><span>${statusBadge(bill.status)} <button type="button" class="btn btn-small btn-ghost" data-document="invoice" data-invoice="${escapeHtml(bill.id)}">Lihat</button></span></div>`;
...
function selfPasswordForm() {
  ...
  const warning = 'Batas maksimal perubahan kata sandi mandiri telah tercapai (2/2 kali). Untuk melakukan perubahan/reset kata sandi kembali, silakan hubungi Admin Maâ€™had.';
...
function invoiceTable(studentId) {
  ...
  if (!studentId && !['parent', 'student'].includes(effectiveRole())) return groupedActiveStudentRecords(invoices, (invoice) => `<div class="accordion-record"><b>${escapeHtml(studentById(invoice.studentId).name)}</b><span>${escapeHtml(invoice.number)} Â· ${money(invoice.total)}</span><span>${statusBadge(invoice.status)} <button class="btn btn-small btn-ghost" data-document="invoice" data-invoice="${invoice.id}">Lihat</button></span></div>`, 'Belum ada data invoice');
...
function activityTimeline() {
  ...
  return `<div class="timeline">${items.map((item) => `<div class="timeline-item"><div class="time">${item.time}</div><div class="timeline-dot">${icon(item.type === 'Tahfizh' ? 'book-open' : 'calendar-days', 16)}</div><div><b>${item.title}</b><p>${item.room || 'Agenda BoardingPro STK IS'} Â· ${item.teacher || ''}</p></div></div>`).join('')}</div>`;
}
...
function formatSanctionForPrint(value) {
  ...
  const listItems = lines.map((line) => line.replace(/^\s*(?:[-*â€¢]|\d+[.)])\s*/, '')).filter(Boolean);
  const isList = lines.some((line) => /^\s*(?:[-*â€¢]|\d+[.)])\s+/.test(line));
  return isList
...
function disciplineFilters(records) {
  ...
  const rows = visible.map((record) => {
    ...
    return `<tr><td><b>${escapeHtml(student.name)}</b><small>${escapeHtml(student.nis || '')}</small></td><td>${escapeHtml(student.className || 'â€”')}</td><td>${escapeHtml(disciplineLevelLabel(record.level))}</td><td>${formatDate(record.incidentDate)}</td><td>${escapeHtml(record.sanction)}</td><td>${statusBadge(disciplineStatus(record))}</td><td>${action}</td></tr>`;
  }).join('');
...
function disciplineView() {
  ...
  return `${welcome('MODUL KEDISIPLINAN SANTRI', 'Surat SP 1 Â· SP 2 Â· SP 3 Â· DO', 'Pencatatan, penerbitan surat, dan monitoring riwayat pembinaan.', '')}${canCreate ? disciplineForm() : ''}${disciplineFilters(records)}`;
}
...
function studentsTable() {
  return table(['Santri', 'Program / Kelas', 'Jurusan','Kehadiran', 'SPP', 'Aksi'], state.students.map((student) => { const programLabel = student.program === 'PPTAK' ? 'Program PPTAK (1 Tahun)' : student.program === 'KWNQ' ? 'Program KWNQ (3 Bulan)' : student.program; const classLabel = ['PPTAK', 'KWNQ'].includes(student.program) ? (student.status === 'Alumni Program' ? 'Alumni Program' : 'Program khusus') : student.className; return `<tr><td><div class="person"><span class="avatar">${initials(student.name)}</span><div><b>${escapeHtml(student.name)}</b><small>${escapeHtml(student.parent)} Â· ${escapeHtml(student.nis)}</small></div></div></td><td>${escapeHtml(programLabel)}<br><small>${escapeHtml(classLabel)}</small></td><td>${escapeHtml((state.majors || majors).find((major) => major.id === student.major)?.name || 'â€”')}</td><td>${student.attendance}%</td><td>${statusBadge(student.spp)}</td><td>${currentRoleIsAdmin() ? `<div class="actions-inline"><button type="button" class="icon-btn" data-edit-student="${student.id}">${icon('pencil', 16)}</button><button type="button" class="icon-btn" data-delete-student="${student.id}" title="Hapus">${icon('trash-2', 16)}</button></div>` : 'â€”'}</td></tr>`; }).join(''));
}
function classesTable() {
  return table(['Program', 'Kelas', 'Jurusan', 'Wali Kelas', 'Santri', 'Aksi'], state.classes.map((klass) => {
    ...
    return `<tr><td>${programLabel}</td><td><b>${klass.programId === 'PPTAK' ? 'Program 1 Tahun' : klass.programId === 'KWNQ' ? 'Program 3 Bulan' : klass.name}</b></td><td>${klass.programId === 'SMK' && major ? major.name : 'â€”'}</td><td>${klass.wali || 'â€”'}</td><td>${klass.studentIds.length}</td><td>${currentRoleIsAdmin() ? `<button class="icon-btn" data-edit-class="${klass.id}">${icon('pencil', 15)}</button>` : 'â€”'}</td></tr>`;
  }).join(''));
}
function pocketRoomSummary(students) {
  ...
  const groupsHtml = Object.entries(groups).sort(([a], [b]) => a.localeCompare(b, 'id')).map(([className, classStudents]) => {
    const rows = classStudents.map((student) => {
      ...
      return `<tr><td><b>${escapeHtml(student.name)}</b><small>${escapeHtml(student.nis || '')}</small></td><td>${escapeHtml(student.room || 'â€”')}</td><td>${money(balance)}</td><td>${money(spent)}</td></tr>`;
    }).join('');
...
function pocketMoneyView() {
  ...
  const expenseForm = (currentRoleIsAdmin() || ['pembina', 'musyrif'].includes(role)) ? `<form id="pocket-expense-form" class="form-grid"><label>Santri<select name="studentId">${managedStudents.map((item) => `<option value="${item.id}">${escapeHtml(item.name)} Â· ${escapeHtml(item.nis)}</option>`).join('')}</select></label><label>Nominal Pengeluaran (Rp)<input name="amount" type="number" min="1" step="1000" required></label><label class="full">Keperluan<input name="note" placeholder="Contoh: kantin, koperasi"></label><button type="submit" class="btn btn-primary full">Catat Pengeluaran</button></form>` : '';
  const pending = state.pocketTransactions.filter((item) => item.type === 'Top Up' && item.status === 'Pending');
  const approval = currentRoleIsAdmin() ? table(['Santri', 'Nominal', 'Tanggal', 'Catatan', 'Aksi'], pending.map((item) => `<tr><td>${escapeHtml(studentById(item.studentId).name)}</td><td>${money(item.amount)}</td><td>${formatDate(item.date)}</td><td>${escapeHtml(item.note || 'â€”')}</td><td><button type="button" class="btn btn-small btn-primary" data-pocket-approve="${item.id}">Verifikasi</button></td></tr>`).join(''), 'Tidak ada top up pending') : '';
  const roomSummary = role === 'pembina' || role === 'musyrif' ? pocketRoomSummary(managedStudents) : '';
  return `${welcome('UANG SAKU SANTRI', 'Saldo & transaksi', 'Top up wali diverifikasi admin, pengeluaran langsung mengurangi saldo.', '')}<div class="finance-grid"><div class="finance-tile"><span>Saldo ${escapeHtml(student.name)}</span><b>${money(balance)}</b></div><div class="finance-tile"><span>Total top up</span><b>${money(transactions.filter((item) => item.type === 'Top Up' && item.status !== 'Pending').reduce((sum, item) => sum + Number(item.amount || 0), 0))}</b></div><div class="finance-tile"><span>Total pengeluaran</span><b>${money(transactions.filter((item) => item.type === 'Pengeluaran').reduce((sum, item) => sum + Number(item.amount || 0), 0))}</b></div></div><div class="grid-2">${isParent ? section('Ajukan Top Up', 'Status awal menunggu verifikasi admin', topUpForm) : ''}${expenseForm ? section('Catat Pengeluaran', 'Hanya admin atau pembina', expenseForm) : ''}</div>${approval ? section('Verifikasi Top Up', 'Persetujuan admin', approval) : ''}${roomSummary}${section('Riwayat Uang Saku', 'Mutasi saldo terbaru', table(['Tanggal', 'Jenis', 'Nominal', 'Status', 'Catatan'], transactions.map((item) => `<tr><td>${formatDate(item.date)}</td><td>${escapeHtml(item.type)}</td><td>${money(item.amount)}</td><td>${item.status ? statusBadge(item.status) : statusBadge('Verified')}</td><td>${escapeHtml(item.note || 'â€”')}</td></tr>`).join(''), 'Belum ada transaksi'))}`;
}
function teachersTable() {
  return table(['Nama', 'Mapel', 'Peran', 'Telepon', 'Status', 'Aksi'], state.teachers.map((teacher) => `<tr><td><b>${teacher.name}</b></td><td>${teacher.subject}</td><td>${teacher.role}</td><td>${teacher.phone || 'â€”'}</td><td>${statusBadge(teacher.status)}</td><td>${currentRoleIsAdmin() ? `<button class="icon-btn" data-edit-teacher="${teacher.id}">${icon('pencil', 15)}</button>` : 'â€”'}</td></tr>`).join(''));
}
...
function announcementsView() {
  ...
  const rows = state.announcements.map((item) => `<tr><td><b>${item.title}</b><small>${item.detail}</small></td><td>${formatDate(item.date)}</td><td>${item.author}</td><td>${canManageAnnouncements() ? `<div class="actions-inline"><button class="icon-btn" data-edit-announcement="${item.id}" title="Edit">${icon('pencil', 15)}</button><button class="icon-btn" data-delete-announcement="${item.id}" title="Hapus">${icon('trash-2', 15)}</button></div>` : 'â€”'}</td></tr>`).join('');
...
function promoteStudents() {
  if (!canPromoteStudents()) {
    alert('Kenaikan kelas hanya dapat diproses oleh Maâ€™had / Admin atau Kepala Sekolah.');
    return;
  }
...
function pklViewWithEligibility() {
  ...
  if (effectiveRole() === 'student' && !isPklEligible(student)) {
    return section('Praktik Kerja Lapangan', 'Modul belum dibuka', `<div class="notice">${icon('lock', 25)}<b>Akses PKL belum tersedia</b><p class="metric-note">PKL hanya dibuka untuk siswa SMK kelas 11 semester 2 atau kelas 12 semester 1. Status Anda saat ini: kelas ${escapeHtml(student.grade || 'â€”')} semester ${escapeHtml(student.semester || 'â€”')}.</p></div>`);
  }
  return pklView();
}
...
function bindActions() {
  ...
  document.querySelectorAll('[data-status]').forEach((button) => button.addEventListener('click', (event) => {
    ...
    const finish = (rejectionReason, approvedDays) => {
      ...
      if (permit.status === 'Approved') {
        ...
        permit.officialSignature = '[ TERVERIFIKASI DIGITAL MAâ€™HAD / PEMBINA ]';
        permit.officialStamp = 'STEMPEL DIGITAL MAâ€™HAD';
      }
      persist();
      render();
    };
  ...
  document.querySelectorAll('[data-action="export"]').forEach((button) => button.addEventListener('click', () => alert('Laporan berhasil disiapkan untuk diunduh.')));
...
function showLanding() {
  ...
  const updatePreview = () => { const role = roles[selector.value]; preview.textContent = `${role.demoName} Â· ${role.description}`; };
...
function renderRoleBasedNotifications() {
  ...
  list.innerHTML = notifications.length
    ? notifications.map((item) => `<article class="notification-item ${item.read ? 'notification-read' : ''}" data-notification-id="${escapeHtml(item.id)}" data-notification-target="${escapeHtml(item.targetView || 'dashboard')}"><div class="notification-item-head"><b>${escapeHtml(item.title)}</b><span class="notification-category">${escapeHtml(item.category)}</span></div><p>${escapeHtml(item.description)}</p><time>${escapeHtml(item.timestamp)}${item.read ? ' Â· Dibaca' : ' Â· Baru'}</time></article>`).join('')
