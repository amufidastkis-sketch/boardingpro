...
function studentDashboard() {
  ...
  return commonDashboard('SANTRI ”¬•– PORTAL PERSONAL', 'Jadwal, nilai, tahfizh, dan pengumuman untuk santri.', `${section('Profil & Progress Saya', `${student.className} ”¬•– ${student.program}`, `${tahfizhTable(student.id)}${gradeTable(student.id)}`)}${section('Jadwal & Event', 'Agenda harian dan kegiatan maÎ“Ã‡Ã–had.', `${activityTimeline()}${eventsTable()}`)}`);
}
...
function unifiedDashboard() {
  ...
  return `${welcome(`BOARDINGPRO STK IS ”¬•– ${role.label.toUpperCase()}`, `Assalamu'alaikum, ${role.demoName.split(' ')[0]} ‰¡Æ’Ã¦Ã¯`, `${formatLocalLongDate()} ”¬•– Satu ruang kendali untuk menjaga amanah.`, canManageAnnouncements() ? `<button class="btn btn-primary" data-action="add-announcement">${icon('plus')} Pengumuman</button>` : '')}
...
function yayasanDashboard() {
  ...
  const pending = state.payments.filter((payment) => payment.status === 'Pending').length;
  return `${welcome(`EXECUTIVE OVERVIEW ”¬•– ${formatLocalLongDate()}`, "Assalamu'alaikum, Pengelola ‰¡Æ’Ã¦Ã¯", 'Ringkasan kinerja dan kesehatan keuangan BoardingPro STK IS.', `<button class="btn btn-primary" data-action="export">${icon('download')} Export Laporan</button>`)}
    ...
    ${section('Monitoring Yayasan ”¬•– Tahfizh', 'Program Î“Ã¥Ã† kelas Î“Ã¥Ã† santri dengan target dan capaian', yayasanAccordion())}`;
}
  ...
  return `<div class="accordion">${programs.map((program, programIndex) => {
    ...
    return `<details class="accordion-item" ${programIndex === 0 ? 'open' : ''}><summary><span><b>${program.name}</b><small>${program.description}</small></span><span class="badge badge-neutral">${classes.length} kelas</span></summary><div class="accordion-body">${classes.map((klass) => {
      ...
      return `<details class="accordion-class"><summary><span><b>${klass.name}</b><small>Wali: ${klass.wali || 'Î“Ã‡Ã¶'} ”¬•– ${students.length} santri</small></span><span class="progress-label">${pct}%</span></summary><div class="accordion-body"><div class="progress"><span style="width:${pct}%"></span></div>${table(['Santri', 'Target (Juz / Surah / Ayat)', 'Capaian (Juz / Surah / Ayat)', 'Mutqin', 'Progress'], students.map((student) => {
        ...
        return `<tr><td><b>${student.name}</b><small>${student.nis}</small></td><td>Juz ${record.targetJuz || 0} ”¬•– ${record.targetSurah || 'Î“Ã‡Ã¶'} ”¬•– ${record.targetAyat || 'Î“Ã‡Ã¶'}</td><td>Juz ${record.achievementJuz || student.tahfizh || 0} ”¬•– ${record.achievementSurah || 'Î“Ã‡Ã¶'} ”¬•– ${record.achievementAyat || 'Î“Ã‡Ã¶'}</td><td>${statusBadge(record.mutqin || 'Belum dinilai')}</td><td><div class="progress"><span style="width:${studentPct}%"></span></div><small>${studentPct}%</small></td></tr>`;
...
function managementView() {
  ...
  return `${welcome('PUSAT KENDALI PENGASUHAN', 'Selamat datang, Admin ‰¡Æ’Ã¦Ã¯', 'Kelola santri, tagihan, dan laporan maÎ“Ã‡Ã–had.', currentRoleIsAdmin() ? `<button class="btn btn-primary" data-action="add-student">${icon('plus')} Tambah Santri</button>` : '')}
...
function academicView() {
  ...
  return `${welcome('AKADEMIK & TAHFIZH TERINTEGRASI', 'Rekap pembelajaran & hafalan ‰¡Æ’Ã´Ãœ', 'Nilai akademik dan capaian tahfizh santri dalam satu laporan.', canEdit ? `<button type="button" class="btn btn-primary" data-action="add-grade">${icon('plus')} Input Nilai</button>` : '')}
...
function studentView() {
  const student = currentStudent();
  return `${welcome('PORTAL SANTRI ”¬•– PERSONAL DASHBOARD', `Assalamu'alaikum, ${student.name.split(' ')[0]}! ‰¡Æ’Ã¦Ã¯`, 'Semangat menjalani aktivitas hari ini.', '')}
...
function gateView() {
  ...
  return `${welcome(`POS JAGA UTAMA ”¬•– ${today}`, 'Kontrol Gerbang ‰¡Æ’Ãœ¬', 'Catat pergerakan santri secara realtime dan aman.')}
...
function tahfizhTable(id) {
  ...
  return groupedStudentRecords(data, (item) => `<div class="accordion-record"><b>${escapeHtml(studentById(item.studentId).name)}</b><span>Juz ${escapeHtml(item.juz)} ”¬•– ${escapeHtml(item.surah)} ”¬•– ${escapeHtml(item.ayat || 'Î“Ã‡Ã¶')}</span>${statusBadge(item.status)}</div>`);
}
...
function gradeTable(studentId) {
  ...
  return groupedStudentRecords(records, (grade) => `<div class="accordion-record"><b>${escapeHtml(studentById(grade.studentId).name)}</b><span>${escapeHtml(grade.subject)} ”¬•– Nilai ${escapeHtml(grade.score)}</span><small>${escapeHtml(grade.note || 'Î“Ã‡Ã¶')}</small></div>`);
}
...
function paymentTable(actions = false, studentId) {
  ...
  const renderPayment = (payment) => `<div class="accordion-record"><b>${escapeHtml(studentById(payment.studentId).name)}</b><span>${escapeHtml(categoryLabel(payment.category))} ”¬•– ${escapeHtml(payment.period || 'Î“Ã‡Ã¶')} ”¬•– ${money(payment.amount)}</span><span>${statusBadge(payment.status)} ${actions && ['Pending', 'pending_verification'].includes(payment.status) ? `<button class="btn btn-small btn-primary" data-payment="${payment.id}" data-payment-status="Verified">Verifikasi</button>` : payment.invoiceId ? `<button class="btn btn-small btn-ghost" data-document="invoice" data-invoice="${payment.invoiceId}">Invoice</button>` : ''}</span></div>`;
...
function attendanceSemesterView() {
  ...
  return `${welcome('REKAP ABSENSI PER SEMESTER', 'Absensi Santri Semester', 'Rekap hadir, izin, sakit, alpha, dan persentase kehadiran.', `<button type="button" class="btn btn-primary" data-action="export-attendance-semester">${icon('download')} Export PDF</button>`)}${section('Filter Semester', 'Filter otomatis dapat dipilih kembali untuk arsip.', semesterFilterForm('attendance-semester-filter'))}${section('Rekap Absensi', `${filter.semester === '1' ? 'Ganjil' : 'Genap'} ”¬•– ${filter.academicYear}`, `<div class="table-responsive table-wrap">${table(['Nama Santri', 'Kelas', 'Total Hadir', 'Izin', 'Sakit', 'Alpha', 'Persentase Kehadiran (%)'], rows)}</div>`)}`;
}
