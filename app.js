...
function securityAlertBanner() {
  ...
  return active.length ? `<div class="notice" style="margin-bottom:18px"><b>ΓÜá ${active.length} laporan darurat keamanan perlu ditindaklanjuti.</b> <button class="btn btn-small btn-ghost" data-view="security-reports">Buka laporan</button></div>` : '';
}
...
function exportReportPdf() {
  ...
  const rows = state.students.map((student) => `<tr><td>${escapeHtml(student.name)}</td><td>${escapeHtml(student.className || student.program || 'ΓÇö')}</td><td>${escapeHtml(String(student.attendance ?? 'ΓÇö'))}%</td><td>${escapeHtml(student.spp || 'ΓÇö')}</td></tr>`).join('');
...
function renderDocumentPreview(docType, docData = {}) {
  ...
  const content = docData.html || ({
    discipline: `<h2>${escapeHtml(docData.level || 'SURAT KEDISIPLINAN')}</h2><p>Santri: <b>${escapeHtml(docData.studentName || 'ΓÇö')}</b></p><p class="document-multiline">${escapeHtml(docData.sanction || docData.description || 'ΓÇö')}</p>`,
    invoice: `<h2>INVOICE PEMBAYARAN</h2><p>Nomor: <b>${escapeHtml(docData.number || 'ΓÇö')}</b></p><p>Santri: ${escapeHtml(docData.studentName || 'ΓÇö')}</p><p class="document-multiline">${escapeHtml(docData.description || 'ΓÇö')}</p><h3>${escapeHtml(docData.total || 'ΓÇö')}</h3>`,
    billing: `<h2>SURAT TAGIHAN</h2><p>Nomor: <b>${escapeHtml(docData.number || 'ΓÇö')}</b></p><p>Santri: ${escapeHtml(docData.studentName || 'ΓÇö')}</p><p class="document-multiline">${escapeHtml(docData.description || 'ΓÇö')}</p><h3>${escapeHtml(docData.total || 'ΓÇö')}</h3>`,
    permit: `<h2>SURAT IZIN SANTRI</h2><p>Santri: <b>${escapeHtml(docData.studentName || 'ΓÇö')}</b></p><p class="document-multiline">${escapeHtml(docData.reason || 'ΓÇö')}</p>`
...
function securityReportsView() {
  const incidentRows = state.incidents.map((item) => `<tr><td><b>${escapeHtml(item.type)}</b><small>${escapeHtml(item.location)}</small></td><td>${statusBadge(item.level)}</td><td>${escapeHtml(item.description)}</td><td>${escapeHtml(item.status || 'Baru')}</td><td>${canManageSecurity() ? `<button class="btn btn-small btn-primary" data-security-status="incident:${item.id}">Tindak Lanjuti</button>` : 'ΓÇö'}</td></tr>`).join('');
  const lostRows = state.lostFound.map((item) => `<tr><td><b>${escapeHtml(item.itemName)}</b><small>${escapeHtml(item.photoDescription)}</small></td><td>${escapeHtml(item.location)}</td><td>${formatDate(item.dateTime?.slice(0, 10) || today)}</td><td>${escapeHtml(item.status)}</td><td>${canManageSecurity() ? `<button class="btn btn-small btn-primary" data-security-status="lost:${item.id}">Konfirmasi</button>` : 'ΓÇö'}</td></tr>`).join('');
...
function scheduleView() {
  ...
  return `${welcome('JADWAL & AGENDA', 'Jadwal Harian dan Event Mendatang', 'Agenda kegiatan yang diperbarui realtime.', actions)}${section('Jadwal Harian', 'Kegiatan, lokasi, dan penanggung jawab.', activityTimeline())}${section('Event Mendatang', 'Agenda resmi sekolah dan maΓÇÖhad.', eventsTable())}`;
}
...
function studentDashboard() {
  ...
  return commonDashboard('SANTRI ┬╖ PORTAL PERSONAL', 'Jadwal, nilai, tahfizh, dan pengumuman untuk santri.', `${section('Profil & Progress Saya', `${student.className} ┬╖ ${student.program}`, `${tahfizhTable(student.id)}${gradeTable(student.id)}`)}${section('Jadwal & Event', 'Agenda harian dan kegiatan maΓÇÖhad.', `${activityTimeline()}${eventsTable()}`)}`);
}
...
function unifiedDashboard() {
  ...
  return `${welcome(`BOARDINGPRO STK IS ┬╖ ${role.label.toUpperCase()}`, `Assalamu'alaikum, ${role.demoName.split(' ')[0]} ≡ƒæï`, `${formatLocalLongDate()} ┬╖ Satu ruang kendali untuk menjaga amanah.`, canManageAnnouncements() ? `<button class="btn btn-primary" data-action="add-announcement">${icon('plus')} Pengumuman</button>` : '')}
    <div class="stats-grid">${stats}</div>
    <div class="grid-2">${section('Pengumuman & Feed Harian', 'Kabar terbaru maΓÇÖhad dan perkembangan santri', `<div class="activity-list">${announcementRows || '<div class="empty">Belum ada pengumuman.</div>'}${feed.map((item) => `<div class="activity"><span class="activity-icon green">${icon(item.type === 'tahfizh' ? 'book-open-check' : 'bell')}</span><div><b>${escapeHtml(item.title)}</b><p>${escapeHtml(item.detail)}</p></div><time>${formatDate(item.date)}</time></div>`).join('')}</div>`, `<button class="btn btn-ghost btn-small" data-view="announcements">Lihat semua</button>`)}
...
function yayasanDashboard() {
  ...
  const pending = state.payments.filter((payment) => payment.status === 'Pending').length;
  return `${welcome(`EXECUTIVE OVERVIEW ┬╖ ${formatLocalLongDate()}`, "Assalamu'alaikum, Pengelola ≡ƒæï", 'Ringkasan kinerja dan kesehatan keuangan BoardingPro STK IS.', `<button class="btn btn-primary" data-action="export">${icon('download')} Export Laporan</button>`)}
    ...
    ${section('Monitoring Yayasan ┬╖ Tahfizh', 'Program ΓåÆ kelas ΓåÆ santri dengan target dan capaian', yayasanAccordion())}`;
}
  ...
  return `<div class="accordion">${programs.map((program, programIndex) => {
    ...
    return `<details class="accordion-item" ${programIndex === 0 ? 'open' : ''}><summary><span><b>${program.name}</b><small>${program.description}</small></span><span class="badge badge-neutral">${classes.length} kelas</span></summary><div class="accordion-body">${classes.map((klass) => {
      ...
      return `<details class="accordion-class"><summary><span><b>${klass.name}</b><small>Wali: ${klass.wali || 'ΓÇö'} ┬╖ ${students.length} santri</small></span><span class="progress-label">${pct}%</span></summary><div class="accordion-body"><div class="progress"><span style="width:${pct}%"></span></div>${table(['Santri', 'Target (Juz / Surah / Ayat)', 'Capaian (Juz / Surah / Ayat)', 'Mutqin', 'Progress'], students.map((student) => {
        ...
        return `<tr><td><b>${student.name}</b><small>${student.nis}</small></td><td>Juz ${record.targetJuz || 0} ┬╖ ${record.targetSurah || 'ΓÇö'} ┬╖ ${record.targetAyat || 'ΓÇö'}</td><td>Juz ${record.achievementJuz || student.tahfizh || 0} ┬╖ ${record.achievementSurah || 'ΓÇö'} ┬╖ ${record.achievementAyat || 'ΓÇö'}</td><td>${statusBadge(record.mutqin || 'Belum dinilai')}</td><td><div class="progress"><span style="width:${studentPct}%"></span></div><small>${studentPct}%</small></td></tr>`;
...
function managementView() {
  ...
  return `${welcome('PUSAT KENDALI PENGASUHAN', 'Selamat datang, Admin ≡ƒæï', 'Kelola santri, tagihan, dan laporan maΓÇÖhad.', currentRoleIsAdmin() ? `<button class="btn btn-primary" data-action="add-student">${icon('plus')} Tambah Santri</button>` : '')}
...
function academicView() {
  ...
  return `${welcome('AKADEMIK & TAHFIZH TERINTEGRASI', 'Rekap pembelajaran & hafalan ≡ƒôÜ', 'Nilai akademik dan capaian tahfizh santri dalam satu laporan.', canEdit ? `<button type="button" class="btn btn-primary" data-action="add-grade">${icon('plus')} Input Nilai</button>` : '')}
...
function teacherPersonalAttendanceView() {
  ...
  const report = table(['Tanggal', 'Masuk', 'Pulang', 'Status'], records.map((item) => `<tr><td>${formatDate(item.date)}</td><td>${item.checkIn || 'ΓÇö'}</td><td>${item.checkOut || 'ΓÇö'}</td><td>${statusBadge(item.status)}</td></tr>`).join(''), 'Belum ada riwayat presensi');
...
function teacherAttendanceTable() {
  ...
  const rows = teachers.map((teacher) => {
    ...
    const subject = items[0]?.subject || 'ΓÇö';
...
function openTeacherAttendanceDetail(teacherName) {
  ...
  const subject = records[0]?.subject || 'ΓÇö';
  const rows = records.map((item) => `<tr><td>${formatDate(item.date)}</td><td>${new Date(`${item.date}T00:00:00`).toLocaleDateString('id-ID', { weekday: 'long' })}</td><td>${statusBadge(item.status)}</td><td>${Number(item.hours || 0)} jam</td><td>${escapeHtml(item.note || 'ΓÇö')}</td></tr>`).join('');
...
function studentView() {
  const student = currentStudent();
  return `${welcome('PORTAL SANTRI ┬╖ PERSONAL DASHBOARD', `Assalamu'alaikum, ${student.name.split(' ')[0]}! ≡ƒæï`, 'Semangat menjalani aktivitas hari ini.', '')}
...
function gateView() {
  ...
  return `${welcome(`POS JAGA UTAMA ┬╖ ${today}`, 'Kontrol Gerbang ≡ƒÜ¬', 'Catat pergerakan santri secara realtime dan aman.')}
...
function permitGroupClass(student) {
  return ['PPTAK', 'KWNQ'].includes(student.program) ? 'Non-Jenjang' : [10, 11, 12].includes(Number(student.grade)) ? `Kelas ${Number(student.grade)}` : 'Kelas ΓÇö';
}
...
function tahfizhTable(id) {
  ...
  return groupedStudentRecords(data, (item) => `<div class="accordion-record"><b>${escapeHtml(studentById(item.studentId).name)}</b><span>Juz ${escapeHtml(item.juz)} ┬╖ ${escapeHtml(item.surah)} ┬╖ ${escapeHtml(item.ayat || 'ΓÇö')}</span>${statusBadge(item.status)}</div>`);
}
...
function gradeTable(studentId) {
  ...
  if (studentId || ['parent', 'student'].includes(effectiveRole())) return table(['Santri', 'Mata Pelajaran', 'Nilai', 'Catatan'], records.map((grade) => `<tr><td>${escapeHtml(studentById(grade.studentId).name)}</td><td>${escapeHtml(grade.subject)}</td><td><span class="score">${grade.score}</span></td><td>${escapeHtml(grade.note || 'ΓÇö')}</td></tr>`).join(''));
  return groupedStudentRecords(records, (grade) => `<div class="accordion-record"><b>${escapeHtml(studentById(grade.studentId).name)}</b><span>${escapeHtml(grade.subject)} ┬╖ Nilai ${escapeHtml(grade.score)}</span><small>${escapeHtml(grade.note || 'ΓÇö')}</small></div>`);
}
...
function pklTable() {
  return table(['Santri', 'Jam Masuk', 'Jam Pulang', 'Status Absensi', 'Deskripsi Kegiatan', 'Foto'], state.pklReports.map((report) => `<tr><td>${escapeHtml(studentById(report.studentId).name)}</td><td>${escapeHtml(report.checkIn || 'ΓÇö')}</td><td>${escapeHtml(report.checkOut || 'ΓÇö')}</td><td>${statusBadge(report.status)}</td><td>${escapeHtml(report.activity || 'ΓÇö')}</td><td>${report.photo ? 'Tersedia' : 'ΓÇö'}</td></tr>`).join(''));
}
...
function selfPasswordForm() {
  ...
  const warning = 'Batas maksimal perubahan kata sandi mandiri telah tercapai (2/2 kali). Untuk melakukan perubahan/reset kata sandi kembali, silakan hubungi Admin MaΓÇÖhad.';
...
function gateTable() {
  ...
  return table(['Santri', 'Jenis Izin', 'Tanggal', 'Jam Keluar', 'Jam Kembali', 'Aksi'], data.map((permit) => `<tr><td><div class="person"><span class="avatar">${initials(studentById(permit.studentId).name)}</span><b>${studentById(permit.studentId).name}</b></div></td><td>${permit.type}</td><td>${formatDate(permit.date)}</td><td>${permit.checkout || 'ΓÇö'}</td><td>${permit.checkin || 'ΓÇö'}</td><td>${permit.checkin ? statusBadge('Selesai') : `<button class="btn btn-small btn-primary" data-gate="${permit.id}">${permit.checkout ? 'Check-in' : 'Checkout'}</button>`}</td></tr>`).join(''));
}
...
function formatSanctionForPrint(value) {
  ...
  if (!lines.length) return 'ΓÇö';
  const listItems = lines.map((line) => line.replace(/^\s*(?:[-*ΓÇó]|\d+[.)])\s*/, '')).filter(Boolean);
  const isList = lines.some((line) => /^\s*(?:[-*ΓÇó]|\d+[.)])\s+/.test(line));
  return isList
...
function renderDisciplineLetter(recordId) {
  ...
  const details = `<table class="info-table">${detailRow('Nama Santri', `<b>${escapeHtml(student.name)}</b>`)}${detailRow('NIS', escapeHtml(student.nis || 'ΓÇö'))}${detailRow('Kelas', escapeHtml(student.className || 'ΓÇö'))}${detailRow('Tanggal Kejadian', formatDate(record.incidentDate))}${detailRow('Jenis Pelanggaran', escapeHtml(record.violationType))}${detailRow('Deskripsi / Sanksi', `<div>${escapeHtml(record.description)}</div><div class="box-sanction">${formatSanctionForPrint(record.sanction)}</div>`, 'info-table--multiline')}${detailRow('Masa Berlaku', `${formatDate(record.issuedAt || record.incidentDate)} s.d. ${formatDate(expirationDate)}`)}${detailRow('Status', escapeHtml(disciplineStatus(record)))}</table>`;
  const statusLabel = verified ? 'Γ£ô Terverifikasi' : 'Draft';
...
function disciplineFilters(records) {
  ...
  const rows = visible.map((record) => {
    ...
    return `<tr><td><b>${escapeHtml(student.name)}</b><small>${escapeHtml(student.nis || '')}</small></td><td>${escapeHtml(student.className || 'ΓÇö')}</td><td>${escapeHtml(disciplineLevelLabel(record.level))}</td><td>${formatDate(record.incidentDate)}</td><td>${escapeHtml(record.sanction)}</td><td>${statusBadge(disciplineStatus(record))}</td><td>${action}</td></tr>`;
  }).join('');
...
function studentsTable() {
  return table(['Santri', 'Program / Kelas', 'Jurusan','Kehadiran', 'SPP', 'Aksi'], state.students.map((student) => { const programLabel = student.program === 'PPTAK' ? 'Program PPTAK (1 Tahun)' : student.program === 'KWNQ' ? 'Program KWNQ (3 Bulan)' : student.program; const classLabel = ['PPTAK', 'KWNQ'].includes(student.program) ? (student.status === 'Alumni Program' ? 'Alumni Program' : 'Program khusus') : student.className; return `<tr><td><div class="person"><span class="avatar">${initials(student.name)}</span><div><b>${escapeHtml(student.name)}</b><small>${escapeHtml(student.parent)} ┬╖ ${escapeHtml(student.nis)}</small></div></div></td><td>${escapeHtml(programLabel)}<br><small>${escapeHtml(classLabel)}</small></td><td>${escapeHtml((state.majors || majors).find((major) => major.id === student.major)?.name || 'ΓÇö')}</td><td>${student.attendance}%</td><td>${statusBadge(student.spp)}</td><td>${currentRoleIsAdmin() ? `<div class="actions-inline"><button type="button" class="icon-btn" data-edit-student="${student.id}">${icon('pencil', 16)}</button><button type="button" class="icon-btn" data-delete-student="${student.id}" title="Hapus">${icon('trash-2', 16)}</button></div>` : 'ΓÇö'}</td></tr>`; }).join(''));
}
function classesTable() {
  return table(['Program', 'Kelas', 'Jurusan', 'Wali Kelas', 'Santri', 'Aksi'], state.classes.map((klass) => {
    ...
    return `<tr><td>${programLabel}</td><td><b>${klass.programId === 'PPTAK' ? 'Program 1 Tahun' : klass.programId === 'KWNQ' ? 'Program 3 Bulan' : klass.name}</b></td><td>${klass.programId === 'SMK' && major ? major.name : 'ΓÇö'}</td><td>${klass.wali || 'ΓÇö'}</td><td>${klass.studentIds.length}</td><td>${currentRoleIsAdmin() ? `<button class="icon-btn" data-edit-class="${klass.id}">${icon('pencil', 15)}</button>` : 'ΓÇö'}</td></tr>`;
  }).join(''));
}
function pocketRoomSummary(students) {
  ...
  const groupsHtml = Object.entries(groups).sort(([a], [b]) => a.localeCompare(b, 'id')).map(([className, classStudents]) => {
    const rows = classStudents.map((student) => {
      ...
      return `<tr><td><b>${escapeHtml(student.name)}</b><small>${escapeHtml(student.nis || '')}</small></td><td>${escapeHtml(student.room || 'ΓÇö')}</td><td>${money(balance)}</td><td>${money(spent)}</td></tr>`;
    }).join('');
...
function pocketMoneyView() {
  ...
  const approval = currentRoleIsAdmin() ? table(['Santri', 'Nominal', 'Tanggal', 'Catatan', 'Aksi'], pending.map((item) => `<tr><td>${escapeHtml(studentById(item.studentId).name)}</td><td>${money(item.amount)}</td><td>${formatDate(item.date)}</td><td>${escapeHtml(item.note || 'ΓÇö')}</td><td><button type="button" class="btn btn-small btn-primary" data-pocket-approve="${item.id}">Verifikasi</button></td></tr>`).join(''), 'Tidak ada top up pending') : '';
  const roomSummary = role === 'pembina' || role === 'musyrif' ? pocketRoomSummary(managedStudents) : '';
  return `${welcome('UANG SAKU SANTRI', 'Saldo & transaksi', 'Top up wali diverifikasi admin, pengeluaran langsung mengurangi saldo.', '')}<div class="finance-grid"><div class="finance-tile"><span>Saldo ${escapeHtml(student.name)}</span><b>${money(balance)}</b></div><div class="finance-tile"><span>Total top up</span><b>${money(transactions.filter((item) => item.type === 'Top Up' && item.status !== 'Pending').reduce((sum, item) => sum + Number(item.amount || 0), 0))}</b></div><div class="finance-tile"><span>Total pengeluaran</span><b>${money(transactions.filter((item) => item.type === 'Pengeluaran').reduce((sum, item) => sum + Number(item.amount || 0), 0))}</b></div></div><div class="grid-2">${isParent ? section('Ajukan Top Up', 'Status awal menunggu verifikasi admin', topUpForm) : ''}${expenseForm ? section('Catat Pengeluaran', 'Hanya admin atau pembina', expenseForm) : ''}</div>${approval ? section('Verifikasi Top Up', 'Persetujuan admin', approval) : ''}${roomSummary}${section('Riwayat Uang Saku', 'Mutasi saldo terbaru', table(['Tanggal', 'Jenis', 'Nominal', 'Status', 'Catatan'], transactions.map((item) => `<tr><td>${formatDate(item.date)}</td><td>${escapeHtml(item.type)}</td><td>${money(item.amount)}</td><td>${item.status ? statusBadge(item.status) : statusBadge('Verified')}</td><td>${escapeHtml(item.note || 'ΓÇö')}</td></tr>`).join(''), 'Belum ada transaksi'))}`;
}
function teachersTable() {
  return table(['Nama', 'Mapel', 'Peran', 'Telepon', 'Status', 'Aksi'], state.teachers.map((teacher) => `<tr><td><b>${teacher.name}</b></td><td>${teacher.subject}</td><td>${teacher.role}</td><td>${teacher.phone || 'ΓÇö'}</td><td>${statusBadge(teacher.status)}</td><td>${currentRoleIsAdmin() ? `<button class="icon-btn" data-edit-teacher="${teacher.id}">${icon('pencil', 15)}</button>` : 'ΓÇö'}</td></tr>`).join(''));
}
...
function announcementsView() {
  ...
  const rows = state.announcements.map((item) => `<tr><td><b>${item.title}</b><small>${item.detail}</small></td><td>${formatDate(item.date)}</td><td>${item.author}</td><td>${canManageAnnouncements() ? `<div class="actions-inline"><button class="icon-btn" data-edit-announcement="${item.id}" title="Edit">${icon('pencil', 15)}</button><button class="icon-btn" data-delete-announcement="${item.id}" title="Hapus">${icon('trash-2', 15)}</button></div>` : 'ΓÇö'}</td></tr>`).join('');
...
function promoteStudents() {
  if (!canPromoteStudents()) {
    alert('Kenaikan kelas hanya dapat diproses oleh MaΓÇÖhad / Admin atau Kepala Sekolah.');
    return;
  }
...
function pklViewWithEligibility() {
  ...
  if (effectiveRole() === 'student' && !isPklEligible(student)) {
    return section('Praktik Kerja Lapangan', 'Modul belum dibuka', `<div class="notice">${icon('lock', 25)}<b>Akses PKL belum tersedia</b><p class="metric-note">PKL hanya dibuka untuk siswa SMK kelas 11 semester 2 atau kelas 12 semester 1. Status Anda saat ini: kelas ${escapeHtml(student.grade || 'ΓÇö')} semester ${escapeHtml(student.semester || 'ΓÇö')}.</p></div>`);
  }
  return pklView();
}

function pklTable() {
  return `<div class="table-responsive table-wrap">${table(['Santri', 'Jam Masuk', 'Jam Pulang', 'Status Absensi', 'Deskripsi Kegiatan', 'Foto'], state.pklReports.map((report) => `<tr><td>${escapeHtml(studentById(report.studentId).name)}</td><td>${escapeHtml(report.checkIn || 'ΓÇö')}</td><td>${escapeHtml(report.checkOut || 'ΓÇö')}</td><td>${statusBadge(report.status)}</td><td>${escapeHtml(report.activity || 'ΓÇö')}</td><td>${report.photo ? `<a href="${escapeHtml(report.photo)}" target="_blank" rel="noopener">Lihat Foto</a>` : 'ΓÇö'}</td></tr>`).join(''), 'Belum ada laporan PKL')}</div>`;
}
function pklView() {
...
function attendanceSemesterView() {
  ...
  const rows = state.students.map((student) => {
    ...
    return `<tr><td>${escapeHtml(student.name)}</td><td>${escapeHtml(student.className || 'ΓÇö')}</td><td>${counts[0]}</td><td>${counts[1]}</td><td>${counts[2]}</td><td>${counts[3]}</td><td>${percent}%</td></tr>`;
  }).join('');
  ...
  document.querySelectorAll('[data-status]').forEach((button) => button.addEventListener('click', (event) => {
    ...
    const finish = (rejectionReason, approvedDays) => {
      ...
      if (permit.status === 'Approved') {
        ...
        permit.officialSignature = '[ TERVERIFIKASI DIGITAL MAΓÇÖHAD / PEMBINA ]';
        permit.officialStamp = 'STEMPEL DIGITAL MAΓÇÖHAD';
      }
      persist();
      render();
    };
